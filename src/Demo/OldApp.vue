    ...
    <div class="main">
      <div class="fixed-top-bar">
        <top-bar 
          ...
          v-bind="$attrs" 
          ref="topBar"
          @changer-nom-document="envoyerDonneesWS"
  ...
  export default {
    ...
    methods: {
      exportToPDF() {
        ...
        dataSend.set("html", this.content);
        dataSend.set("infoFile", localStorage.getItem(this.id_doc));
        dataSend.set("fileName", useFileNameStore().getNomFichier());
  
        let xhr = new XMLHttpRequest();
        xhr.open("POST", CONSTANTES.baseUrl + "/Edt/Pdf/receiveData", true);
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4) {
            if (xhr.status === 200) { 
              ...
              if (res.errCode === 1) {
                open(
                  CONSTANTES.baseUrl + "/Export/" + useFileNameStore().getNomFichier() + ".pdf",
                  "_blank"
                );
              } else {
                console.log("Erreur de l'API : " + res.errorMessage);
              }
            } else {
      ...
      toggleHeader(tag : any) {
        document.execCommand('formatBlock', false, `<${tag}>`);
        this.normalizeHeader();
      },
        
      normalizeHeader() {
        const editableDiv = this.$refs.editor;
        const headers = ["h1", "h2", "h3"];
        headers.forEach((header) => {
          const elements = editableDiv.querySelectorAll(header);
          elements.forEach((element : any) => {
            if (element.parentNode.tagName.toLowerCase() === header) {
        ...
        data.set("util_id", window["util_id"]);
        data.set("util_img", window["util_img"]);
        data.set("util_nom", window["util_nom"]);
        data.set("util_prenom", window["util_prenom"]);
        data.set("util_surnom", window["util_surnom"]);
        data.set("util_mail", window["util_mail"]);
        data.set("client", window["client"].toString());
        data.set("fileCrypted", window.location.pathname.split("/")[2]);
  
        let ajax = new XMLHttpRequest();
        ajax.onload = function (e) {
          const response = JSON.parse(this.response);
            that.listUserConnected = response.listUser;
        };
  
        ajax.open("POST", CONSTANTES.urlApi + "/WSMdoc/saveConnectedUser", false);
        ajax.send(data);
      ...
      getConnectedUser() {
        ...
        let data = new FormData();
        data.set("fileCrypted", window["link"][2]);
        ...
        ajax.open("POST", CONSTANTES.urlApi + "/WSMdoc/getConnectedUser", false);
        ajax.send(data);
      },
      ...
      handleOfflineStatus() {
        // L'application est hors ligne, stockez les modifications localement (LocalStorage).
  
      },
  
      initialiseWS(){
        const url = window.location.pathname.split("/");
        const ressource = url[2];
        const websocketURL = `${CONFIGSOCKET.serverWebSocketAddress}?util_id=${window["util_id"]}&ressource=${ressource}`;
        this.socket = new WebSocket(websocketURL);
        console.log(this.socket)
        const that = this;
  
        this.socket.onopen = function () {
          that.saveConnectedUser();
        };
        ...
        this.socket.onmessage = function (event) {
          ...
          try {
            ...
            const elements =  datas?.message?.contenus?.elements || [datas];
  
            // change le nouveau nom du fichier à tous les users
            that.nomDocument = datas.message.nomFichier;
  
            // format de la page de tous les autres users
            that.page_format_mm = datas.message.format
            that.display = datas.message.display
            that.page_margins = datas.message.marge
            
            if (that.id_doc === datas.message.id) // si c'est le meme document, on peut recevoir des messages
            {
              that.content = [];
              for (let i = 0; i < elements.length; i++) {
                that.content.push(elements[i]);
              }
            }
      ...
      async envoyerDonneesWS() {
        const donnees = await this.typeDonneeSauvegarder(this.content);
        const messageJSON = JSON.stringify(donnees);
      ...
      async fetchData() {
        const contentData = this.contentFile.chargeContenue();
        const result = contentData && contentData.content && contentData.content.hasOwnProperty('dataFile')
          ? contentData.content.dataFile
          : contentData && contentData.content;
  
        if (result) {
          const data = JSON.parse(result)
          // charger format de la page 
          this.page_format_mm = data.format
          this.display = data.display
          this.page_margins = data.marge
  
          // charger le contenu de la page
          const elements =  data?.contenus?.elements || [data];
          this.content = [];
      ...
      async enregistrerDonnees(element : any) {
          const typeData = await this.typeDonneeSauvegarder(element);
          if (!typeData) {
            return;
          }
  
          const donnees = JSON.stringify(typeData);
          localStorage.setItem(this.id_doc, donnees);
  
          const data = new FormData();
          data.set("fic_id", this.id_doc);
          data.set("fic_json", localStorage.getItem(this.id_doc));
          data.set("plateforme", localStorage.getItem("plateforme"));
  
          const xhr = new XMLHttpRequest();
          const url = CONSTANTES.urlApi + "/WSMdoc/saveDoc";
          xhr.open("POST", url);
      ...
      async typeDonneeSauvegarder(element : any) {
        const elements = element
        const jsonData = {
          id: this.id_doc,
          id_ged: this.id_ged,
          base: this.base,
          nomFichier: useFileNameStore().getNomFichier(),
          type: this.type,
          derniereModification: this.derniereModification,
          format : Object.values(this.page_format_mm),
          marge : this.page_margins,
